...
def patchArborescence(patchFilePath:str, arboFilePath:str, pathFilePath:str):
    with open(patchFilePath, mode="r", encoding="UTF-8") as patchFile, \
        open(arboFilePath, mode="r+", buffering=1, encoding="UTF-8") as arboFile, \
        open(pathFilePath, mode="r", encoding="UTF-8") as pathFile:
        ...
        # on passe les bytes d'identification des fichiers
        arboFile.seek(3)
        patchFile.seek(3)
        ...
        with open(TEMP_FILE, mode="a", encoding="UTF-8") as temp:
            ...
            while True:
                ...
                if increment == 0:
                    skip = True # pour le cas de la 1ere ligne
                else:
                    skip = False
                ...
                try:
                    ...
                    break # nous sommmes à la fin du path

                if not skip:
                    temp.write(line)

                # la ligne suivante est la cible de cette étape du path
                line = arboFile.readline()
            ...
            # on a atteint la position voulue du patch, et 'line' contient la ligne suivant le patch
...
def patchFolderOfArborescence(patchFilePath:str, arboFilePath:str, pathFilePath:str):
    with open(patchFilePath, mode="r", encoding="UTF-8") as patchFile, \
        ...
        with open(TEMP_FILE, mode="a", encoding="UTF-8") as temp:
            ...
            # on a atteint la position voulue du patch, et 'line' contient le nom du dossier à patch
